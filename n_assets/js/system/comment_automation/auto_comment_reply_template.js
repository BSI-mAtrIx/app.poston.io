 ...
 $(document).ready(function(){
        ...
        columnDefs: [
            ...
            },
            {
                ...
                "render": function ( data, type, row, meta ) 
                {
                    ...
                    var str="";   
                    str="&nbsp;<a class='text-center edit_reply_info btn btn-circle btn-outline-warning edit' href='#' name='edit' title='"+edit_str+"' id='"+id+"'>"+'<i class="bx bx-edit"></i>'+"</a>";
                    str=str+"&nbsp;<a name='delete' href='#' class='text-center delete_reply_info btn btn-circle btn-outline-danger delete' title='"+delete_str+"' id="+id+" '>"+'<i class="bx bx-trash"></i>'+"</a>";
    ...
    {     
        ...
        output += '<div class="card-header"><h4 class="modal-title text-center"><i class="bx bx-comment"></i> '+AutoComment+'</h4></div> <div class="card-body"><textarea type="text" name="auto_reply_comment_text[]" id="auto_reply_comment_text_'+x+'" class="form-control name_list w-100 height_70px" placeholder="'+AddComments+'"></textarea><span class="clearfix"><a href="#" class="font_size_10px text-center  btn btn-sm btn-outline-danger remove_field float-right clearfix"><i class="bx bx-time"></i> '+remove+'</a></span></div>';
        output += '</div>';
    ...
    $(document).on('submit', '#add_name', function(event) {
        ...
        if(name === '' || comment_block_num === 0 || (comment_block_num === 1 && comment_block_content == '')){
            swal.fire(global_lang_warning, Youdidntprovideallinformation, 'warning');
            return false;
        }
        ...
        if(auto_reply_comment_text == ''){
            swal.fire(global_lang_warning, Youdidntprovideallcomment, 'warning');
            return false;
        }
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                if(action == 'insert')
                {
                    swal.fire(global_lang_success, global_lang_saved_successfully, 'success').then((value) => {
    ...
    $(document).on('click', '.edit', function(e){
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                $('#action').val('edit');
                $('.modal-title').html("<i class='bx bx-comment'></i> "+instagram_auto_comment_template_autocomment);
                $('.modal-header .modal-title').html("<i class='bx bx-comment'></i> "+instagram_auto_comment_template_please_give_the_following_information_for_post_auto_comment);
    ...
    $(document).on('click','.delete',function(e){
        ...
        {
            swal.fire(global_lang_warning, instagram_auto_comment_template_can_not_delete_from_admin, 'warning');
            return;
        }
        e.preventDefault();
        swal.fire({
