...
var iPhoneInstallOverlay = (function(document, localStorage) {
  ...
  <div class=\"add-to-home\">\
    <div class = \"text-right browser-preview f12\" ng-click=\"main.iPhoneNotFullscreen=false\"><button class='btn btn-danger text-bold-500'>{previewText}</button></div>\
    <div class = \"logo-name-container\" style=\"background-image:url({appIconURL})\">\
    </div>\
    <div class='app-name-cont'>{appName}</div>\
    <div class = \"homescreen-text\">{installText}\
      <div class = \"icon-addToHome sprite-mobile\" style=\"background-image:url({spritesURL})\"></div> {chooseText}\
      <br /> {addText}\
    </div>\
