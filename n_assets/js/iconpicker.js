...
;(function($){ "use strict";
    ...
    Iconpicker.DEFAULTS = {
        ...
        arrowNextIconClass: 'bx bx-right-arrow',
        arrowPrevIconClass: 'bx bx-left-arrow',
        cols: 4,
        icon: '',
    ...
    Iconpicker.prototype.select = function (icon) {
        ...
        if (icon !== '' && op.selected >= 0) {
            ...
            if(op.inline === false){
                ...
                el.find('i').attr('class', 'livicon-evo livicon-evo-holder').attr('data-icon',icon);

                var $this = el.find('i'),
                    iconStyle = $("#main-menu-navigation").data("icon-style");

                $this.updateLiviconEvo({
                    name: icon,
                    style: iconStyle,
                    duration: 0.85,
                    strokeWidth: "1.3px",
    ...
    Iconpicker.prototype.updateIcons = function (page) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            for (var j = 0; j < op.cols; j++) {
                ...
                var btn = $('<button class="btn ' + op.unselectedClass + ' btn-icon liv_select " ></button>').hide();
                if (pos < op.icons.length) {
                    var v = op.iconClassFix + op.icons[pos];
                    btn.val(v).attr('title', v).append('<i class=" livicon-evo livicon-evo-holder" data-icon="'+v+'" style="visibility: visible; height: 24px; width: 24px; padding:0!important;top:0;"></i>').show();
                    if (op.icon === v) {
