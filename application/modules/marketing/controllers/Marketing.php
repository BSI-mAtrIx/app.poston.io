...
class Marketing extends Home
{
    ...
    public function get_interest_search_table()
    {
        $iid = 0;
        ...
        $token = $this->basic->get_data("facebook_rx_fb_user_info", array("where" => array("user_id" => $this->user_id)));
        ...
        if (!empty($token[0]['access_token'])) {
            ...
            $url = "https://graph.facebook.com/v18.0/search?" . $ad_type . "&q={$domain_name}&access_token={$token[0]['access_token']}&limit=950&locale={$language}";
            ...
            if (!empty($data['data'])) {
                ...
                if ($action_button == "get_csv") {
                    $this->get_csv($data, $ad_type_switch, $ad_type);
                }
                ...
                switch ($ad_type_switch[0]) {
                    ...
                    case 'type=adinterestsuggestion';
                        if (isset($data['data'][0]['audience_size_lower_bound'])) {
                            $size_type = $this->lang->line('Audience Size');
                        } elseif (isset($data['data'][0]['audience_size'])) {
                            $size_type = $this->lang->line('Audience Size');
                        } else {
                            $size_type = $this->lang->line('Coverage Size');
                        }
		    ...
		    <div class="col-sm-2 text-left">' . $this->lang->line('Topic') . '</div>
                    ...
                    default;
                        ...
                        $keywords = '<div class="row" style="border-bottom:1px solid #000; font-weight:bold;">
                                        <div class="col-sm-6 text-left">' . $this->lang->line('Keywords') . '</div>
                                        <div class="col-sm-6 text-left">' . $size_type . '</div>
                                    </div>';
                ...
                usort($data['data'], function ($a, $b) {
                    ...
                    if (isset($v['audience_size'])) {
                        $size_check = 'audience_size';
                    } elseif (isset($v['coverage'])) {
                        $size_check = 'coverage';
                    } elseif (isset($v['audience_size_lower_bound'])) {
                        $size_check = 'audience_size_lower_bound';
                    } elseif (isset($v['coverage_lower_bound'])) {
                        $size_check = 'coverage_lower_bound';
                    } else {
                        $size_check = '--';
                    }

                    if (!isset($a[$size_check])) {
                        $a[$size_check] = 0;
                    }
                    if (!isset($b[$size_check])) {
                        $b[$size_check] = 0;
                    }
                    if ($a[$size_check] == $b[$size_check]) {
                        return 0;
                    }
                    return ($a[$size_check] > $b[$size_check]) ? -1 : 1;
                });
                ...
                foreach ($data['data'] as $k => $v) {
                    ...
                    switch ($ad_type_switch[0]) {
                        ...
                        case 'type=adinterestsuggestion';
                            ...
                            if (isset($v['audience_size_lower_bound'])) {
                                $size = $v['audience_size_lower_bound'] . ' - ' . $v['audience_size_upper_bound'];
                            } elseif (isset($v['audience_size'])) {
                            ...
                            if (isset($v['path'][0])) {
                                $cat = $v['path'][0] . (isset($v['path'][1]) ? ' => ' . $v['path'][1] : '') . (isset($v['path'][2]) ? ' => ' . $v['path'][2] : '') . (isset($v['path'][3]) ? ' => ' . $v['path'][3] : '') . (isset($v['path'][4]) ? ' => ' . $v['path'][4] : '');
                            } else {
                                $cat = "--";
                            }
                            ...
                            $data_topic = (isset($v['topic']) ? $v['topic'] : "") . (isset($v['disambiguation_category']) ? $v['disambiguation_category'] : "");
		                  ...
		                  />',
                                ...
                                'keyword' => '<span id="copy_' . $v['id'] . '" class="tooltipcs" onclick="copyToClipboard(\'span#copy_' . $v['id'] . '\')" data-toggle="tooltip" title="" data-original-title="' . $this->lang->line('Click to copy') . '">' . $name . '</span> <a target="_blank" href="' . $url_fb . '"><i class="fa fa-facebook-square bx bxl-facebook-circle"></i></a><a target="_blank" href="' . $url_google . '"><i class="fa fa-google bx bxl-google-plus-circle"></i></a>',

                                'size' => $size,
                                'cat' => $cat,
                                'topic' => (isset($v['topic']) ? $v['topic'] : '') . (isset($v['disambiguation_category']) ? ' (' . $v['disambiguation_category'] . ')' : ''),
                            );
                        ...
                        case 'type=adgeolocation';
		                  ...
		                  />',
                                ...
                                'keyword' => '<span id="copy_' . $v['id'] . '" class="tooltipcs" onclick="copyToClipboard(\'span#copy_' . $v['id'] . '\')" data-toggle="tooltip" title="" data-original-title="' . $this->lang->line('Click to copy') . '">' . $name . '</span>',
...
//                        </div><span id="copy_'.$v['id'].'" class="tooltipcs" onclick="copyToClipboard(\'span#copy_'.$v['id'].'\')" data-toggle="tooltip" title="" data-original-title="'.$this->lang->line('Click to copy').'">'.$name.'</span></div><div class="col-sm-6 text-left">'.$size.'</div></div>';
                        ...
                        default;
                            ...
                            if (isset($v['audience_size'])) {
                                ...
                                $size = $v['coverage_lower_bound'] . ' - ' . $v['coverage_upper_bound'];
                            } else {
                                $size = '--';
                            }
...
//
                ...
                echo json_encode(array('status' => '0', 'message' => $this->lang->line('Put only one keyword or not found'), 'count' => 0));
                exit();
            }

            echo json_encode(array('status' => '1', 'message' => "<i class='fa fa-check'></i> " . $this->lang->line('Keywords downloaded.'), 'data' => $ret_table, 'count' => $counter_list, 'audience_size' => $size_type
//            ,'debug'=>json_decode($results, true)
            ));
    ...
    public function get_interest_search()
    {
        ...
        if (!empty($token[0]['access_token'])) {
            ...
            $debug = $ad_type;
            $url = "https://graph.facebook.com/v16.0/search?" . $ad_type . "&q={$domain_name}&access_token={$token[0]['access_token']}&limit=950&locale={$language}";
//echo $url;
            $results = $this->run_curl_for_fb($url);
...
//                        $keyword[$size_type] = $size;
                    ...
                    if (isset($data['data'][0]['audience_size_lower_bound'])) {
                        $keyword['Audience Size Lower Bound'] = $v['audience_size_lower_bound'];
                        $keyword['Audience Size Upper Bound'] = $v['audience_size_upper_bound'];
                    } elseif (isset($data['data'][0]['audience_size'])) {
                        $keyword['Audience Size'] = $v['audience_size'];
                    } else {
                        $keyword['Coverage Size'] = '--';
                    }
...
//             1=> "INSERT INTO `menu` (`id`, `name`, `icon`, `url`, `serial`, `module_access`, `have_child`, `only_admin`, `only_member`, `add_ons_id`, `is_external`, `header_text`, `is_menu_manager`, `custom_page_id`) VALUES (NULL, 'Interest Explorer', 'fa fa-layer-group', 'marketing/get_interest', '1013', '3003', '0', '0', '0', '0', '0', '', '0', '0')",
        ...
        $menu_exists = $this->db->query(" SELECT id FROM `menu` where url LIKE '%marketing/get_interest%' ")->row_array();
        $parent_id_to_add = $this->db->query(" SELECT serial FROM `menu` where url LIKE '%search_tools%' ")->row_array();
        if (!$menu_exists) {
            try {
                $sql_cust = "INSERT INTO `menu` (`id`, `name`, `icon`, `url`, `serial`, `module_access`, `have_child`, `only_admin`, `only_member`, `add_ons_id`, `is_external`, `header_text`, `is_menu_manager`, `custom_page_id`) VALUES (NULL, 'Interest Explorer', 'fa fa-layer-group', 'marketing/get_interest', " . $parent_id_to_add['serial'] . ", '3003', '0', '0', '0', '0', '0', '', '0', '0')";
    ...
    {
        $sql_cust = "DELETE from `menu` where module_access = 3003";
        $this->db->query($sql_cust);

        $sql_cust = "DELETE from `menu_child_1` where url like '%marketing/get_interest%' ";
        $this->db->query($sql_cust);
