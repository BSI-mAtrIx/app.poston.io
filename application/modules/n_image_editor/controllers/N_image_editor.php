<?php
/*
Addon Name: NVX Pixie image editor integrator
Unique Name: nvx_pixie_xero
Modules:
{
   "3004":{
...
Version: 1.51
Description: Image editor N1
*/


require_once("application/controllers/Home.php"); // loading home controller
class N_image_editor extends Home
{
    public $key = "CF4D1CF64203EF88";
	private $product_id = "2";
	private $product_base = "nvx_pixie_xero";
	private $server_host = "https://nvxgroup.com/wp-json/licensor/";
	private $nvx_version = 1.51;
	/* @var self*/
        ...
        if($function_name!="webhook_callback")
        {
             // all addon must be login protected
              ...
              //-------------------------------------------------------------------------------------------
                ...
                if($this->session->userdata('user_type') != 'Admin' && !in_array(3004,$this->module_access))
                   {
    ...
    public function editor(){
        if($this->session->userdata('user_type') != 'Admin' && !in_array(3004,$this->module_access)) redirect('home/login', 'location');
        $data['body'] = 'editor';
	...
	public function activate()
    {
        ...
        $menu_exists = $this->db->query(" SELECT id FROM `menu` where url LIKE '%n_image_editor%' ")->row_array();
        $parent_id_to_add = $this->db->query(" SELECT serial FROM `menu` where url LIKE '%ultrapost%' ")->row_array();
        if(!$menu_exists){
            try{
                $sql_cust = "INSERT INTO `menu` (`id`, `name`, `icon`, `url`, `serial`, `module_access`, `have_child`, `only_admin`, `only_member`, `add_ons_id`, `is_external`, `header_text`, `is_menu_manager`, `custom_page_id`) VALUES (NULL, 'Image Editor N1', 'fa fa-image', 'n_image_editor/', ".$parent_id_to_add['serial'].", '3004', '0', '0', '0', '0', '0', '', '0', '0')" ;
    ...
    public function fix_menu(){
        $sql_cust = "DELETE from `menu_child_1` where url like '%n_image_editor%' " ;
        $this->db->query($sql_cust);
    ...
    public function delete(){        
        ...
        $sql=array
        (
          0 => "DELETE from `menu` where url like '%n_image_editor%' ",
        );  
