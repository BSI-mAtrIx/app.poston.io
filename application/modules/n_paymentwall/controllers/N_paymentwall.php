<?php
/*
Addon Name: Paymentwall Payment Gateway addon for NVX Theme Dashboard
Unique Name: n_paymentwall
Project ID: 1104
Addon URI: https:/nvxgroup.com
Author: Mario Devado
Author URI: https:/nvxgroup.com
Version: 1.3
Description: Paymentwall Payment Gateway
*/
require_once("application/controllers/Home.php"); // loading home controller
include("application/libraries/Facebook/autoload.php");
include(APPPATH.'n_views/include/function_class.php');

class N_paymentwall extends Home
{
    public $key = "16EC809240DF32BD";
    private $product_id = "11";
    private $product_base = "n_paymentwall";
    private $server_host = "https://nvxgroup.com/wp-json/licensor/";
    private $nvx_version = 1.3;
    /* @var self */
    ...
    public function __construct()
    {
        ...
        if (
            ...
            and $function_name != "n_payment_button_generate"
...
//                    }
    ...
    public function n_payment_button($package_id = 0)
    {
        ...
        require_once(APPPATH . 'modules/n_paymentwall/lib/paymentwall.php');
...
//        $success_url=base_url()."n_paymongo/n_payment_button_gcash_return";
        ...
        $n_api_sec = isset($n_config['n_paymentwall_sec']) ? $n_config['n_paymentwall_sec'] : '';
        $n_api_pub = isset($n_config['n_paymentwall_pub']) ? $n_config['n_paymentwall_pub'] : '';
...
//        }
        ...
        $pingback_url = base_url() . "n_paymentwall/webhook";

        Paymentwall_Config::getInstance()->set(array(
            'api_type' => Paymentwall_Config::API_GOODS,
            'public_key' => $n_api_pub,
            'private_key' => $n_api_sec
        ));
        ...
        $widget = new Paymentwall_Widget(
            ...
            array(
                new Paymentwall_Product(
                    $insert_id, // ag_external_id
                    $price, // amount
                    $currency, // currencyCode
                    $package_data[0]["package_name"], // ag_name
...
//    public function n_payment_button_return(){
//        include(FCPATH.'application/n_views/config.php');
//
    ...
    public function webhook()
    {
        ...
        if ($pingback->validate()) {
            ...
            if ($pingback->isDeliverable()) {
                $returjs = array();
                $returjs['type'] = 'paymentwall';
                $returjs['amount'] = $pingback->getProduct()->getAmount();
                $returjs['created_at'] = $curtime;
                $returjs['transaction_id'] = $pingback->getReferenceId();
                $returjs['status'] = 'PAID';
                $this->payment_success($productId, $returjs);
...
//                    $user_email = $this->basic->get_data('users',$where,$select='');
    ...
    public function n_payment_button_ecommerce($store_id = '', $cart_id = '', $subscriber_id = '', $payment_amount = '', $type = 'paymentwall')
    {

        $ecommerce_config = $this->get_ecommerce_config($store_id);
        $n_api_sec = isset($ecommerce_config['n_paymentwall_sec']) ? $ecommerce_config['n_paymentwall_sec'] : '';
        $n_api_pub = isset($ecommerce_config['n_paymentwall_pub']) ? $ecommerce_config['n_paymentwall_pub'] : '';
        ...
        $payment_gateway = $this->n_payment_button_generate_ecommerce($send_arr);
    ...
    private function n_payment_button_generate_ecommerce($send_arr)
    {
        ...
        $pingback_url = base_url() . "n_paymentwall/webhook_ecommerce";
        require_once(APPPATH . 'modules/n_paymentwall/lib/paymentwall.php');

        Paymentwall_Config::getInstance()->set(array(
            'api_type' => Paymentwall_Config::API_GOODS,
            'public_key' => $send_arr['pub_key'],
            'private_key' => $send_arr['sec_key']
        ));

        $widget = new Paymentwall_Widget(
            $send_arr['subscriber_id'], // uid
            'p1_1', // widget
            array(
                new Paymentwall_Product(
                    $send_arr['cart_id'], // ag_external_id
                    $payment_amount, // amount
                    $send_arr['currency'], // currencyCode
                    $shopname . ' Cart ID: ' . $send_arr['cart_id'], // ag_name
                    Paymentwall_Product::TYPE_FIXED // ag_type
                )
            ),
            array(
                ...
                'ps' => 'all', // Replace it with specific payment system short code for single payment methods
                'success_url' => $success_url,
                'failure_url' => $send_arr['cancel_url'],
                'pingback_url' => $pingback_url
            )
        );
...
//    }
    ...
    public function webhook_ecommerce()
    {
        ...
        $order_info = $this->basic->get_data("ecommerce_cart", array("where" => array("id" => $_GET['goodsid'])));
    ...
    public function activate()
    {
        ...
        $sql = array
        (
            0 => "ALTER TABLE `ecommerce_config` ADD `n_paymentwall_sec` VARCHAR(255) NOT NULL DEFAULT '', ADD `n_paymentwall_pub` VARCHAR(255) NOT NULL DEFAULT '';",

            1 => "ALTER TABLE `ecommerce_store` ADD `n_paymentwall_enabled` ENUM('0','1') NOT NULL DEFAULT '0';"
        );
