<?php include(APPPATH . 'modules/n_task/views/main_js.php'); ?>
<script>

    <?php
    include(APPPATH . 'modules/n_task/views/spectrum/spectrum.min.js');
    ...
    $('.todo_ul_edit_mode').on("click", "li", function (e) {
        ...
        $.ajax({
            url: base_url + "n_task/update_field/tasks_todo/status/" + new_value + "/id/" + current_todo_id,
            dataType: 'json',
            cache: false,
    ...
    $('.todo_ul_edit_mode ').on("click", ".close", function (e) {
        ...
        $.ajax({
            url: base_url + "n_task/delete_j/tasks_todo/id/" + current_todo_id,
            dataType: 'json',
            cache: false,
            success: function (data) {
                parent_li.remove();
    ...
    var myDropzone = new Dropzone("#dropzone_form", {
        url: "<?php echo base_url(); ?>n_task/upload_attachments",
        dictDefaultMessage: ""
    });
    ...
    $('#delete_task').on('click', function (event) {
        ...
        if (result) {
            $.ajax({
                url: base_url + "n_task/delete_j/tasks/task_id/" + task_id,
                dataType: 'json',
                cache: false,
                success: function (data) {
    ...
    $('#delete_board').on('click', function (event) {
        ...
        if (result) {
            $.ajax({
                url: base_url + "n_task/delete_board/" + <?php echo $board_id_active; ?>,
                dataType: 'json',
                cache: false,
                success: function (data) {
                    window.location.reload();
    ...
    $('#editTaskModal').on('show.bs.modal', function (event) {
        ...
        $.ajax({
            url: base_url + "n_task/get_task_details/" + current_task_id,
            dataType: 'json',
            cache: false,
            success: function (data) {
                ...
                if (data.task_todo.length > 0) {
                    data.task_todo.forEach(function (a) {
                        if (a.status == 0) {
                            modal.find('.todo_ul').append("<li data-todoid='" + a.id + "'>" + a.title + "<span class='close'><i class='fas fa-times'></i></span></li>");
                        } else {
                            modal.find('.todo_ul').append("<li data-todoid='" + a.id + "' class='checked'>" + a.title + "<span class='close'><i class='fas fa-times'></i></span></li>");
                        }
                    });
                }
    ...
    function popolate_attachment(a) {
        $('.attachments_body').append("<tr><td><i class='fa fa-file'></i></td><td><a href='<?php echo base_url(); ?>uploads/" + a.attachment_filename + "'>" + a.attachment_original_filename + "</a></td><td>" + a.name + "</td><td>" + a.attachment_creation_date + "</td><td><butto class='btn delete_attachment' rel='" + a.attachment_id + "' title='Delete file'><i class='bx bx-trash'></i></button></tr>");
        $('.delete_attachment').on('click', function (event) {
            var result = confirm("Are you sure?");
            var attachment_id = $(this).attr("rel");
            if (result) {
                $.ajax({
                    url: base_url + "n_task/delete_j/attachments/attachment_id/" + attachment_id,
                    dataType: 'json',
                    cache: false,
                    success: function (data) {
                        window.location.reload();
    ...
    $(function () {
        ...
        $(".sortable").sortable({
            ...
            start: function (event, ui) {
                slider.addEventListener('mousemove', startDrag_move_invert, false);
            ...
            stop: function (event, ui) {
                slider.removeEventListener('mousemove', startDrag_move_invert, false);
                slider.addEventListener('mousemove', startDrag_move, false);
                ...
                if ($(ui.item.parent()[0]).attr('rel') == 'bin') {
                    ...
                    $.ajax({
                        url: base_url + "n_task/delete/tasks/task_id/" + task_id,
                        type: 'post',
                        dataType: 'json',
                        data: myArguments,
                        cache: false
                    ...
                    $.ajax({
                        url: base_url + "n_task/update_field/tasks/task_archived/1/task_id/" + task_id,
                        type: 'post',
                        dataType: 'json',
                        data: myArguments,
                        cache: false
                    ...
                    $.ajax({
                        url: base_url + "n_task/update_position",
                        type: 'post',
                        dataType: 'json',
                        data: myArguments,
                        cache: false
        ...
        $('.time_tracker_action').on("click", function () {
            ...
            if (current_task_tracking == null) {
                ...
                $('#timer').runner('start');
                $.ajax({
                    url: base_url + "n_task/time_tracker/start/" + task_id,
                    type: 'get',
                    dataType: 'json',
                    cache: false,
                    success: function (data) {
                ...
                $('#timer').runner('stop');
                $.ajax({
                    url: base_url + "n_task/time_tracker/stop/" + task_id,
                    type: 'get',
                    dataType: 'json',
                    cache: false
                });
        ...
        const slider = document.querySelector('.kanban-container');
        let mouseDown = false;
        let startX, scrollLeft, scrollRight;
        ...
        let startDrag_move_invert = function (e) {
            e.preventDefault();
            if (!mouseDown) {
                return;
            }
...
// Add the event listeners
        ...
        new PerfectScrollbar(".kanban-container", {
            suppressScrollY: true,
            wheelPropagation: false,
        });

        new PerfectScrollbar(".kanban-board main", {
        ...
        $('#select_board_open').on('change', function () {
            window.location.assign(base_url + 'n_task/board/' + this.value);
        });
        ...
        $('.sortable_container #handle-list-1').sortable({
            ...
            update: function (event, ui) {
                ...
                $.ajax({
                    url: base_url + "n_task/update_containers_position",
                    type: 'post',
                    dataType: 'json',
        ...
        $('#deleteContainerModal').on('show.bs.modal', function (event) {
            ...
            $.ajax({
                url: base_url + "n_task/get_container_details/" + container_id,
                dataType: 'json',
                cache: false,
                success: function (data) {
