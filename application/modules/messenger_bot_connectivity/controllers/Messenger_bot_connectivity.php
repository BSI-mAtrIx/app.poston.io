...
class Messenger_bot_connectivity extends Home
{
  public $addon_data=array();
	...
	*/
    ...
    public function edit_webview($id = null,$iframe='0')
    {
    	if($this->session->userdata('user_type') != 'Admin' && !in_array(261,$this->module_access))  redirect('home/login_page', 'location');
    ...
    {           
        ...
        $where = [
        	'where' => [
        		...
        		'webview_builder.page_id' => $page_id,
        	],
        ];
        ...
        for ($i = 0; $i < sizeof($info); $i++) {
            ...
            if (!isset($info[$i]['actions'])) {
                ...
                $actions .= '<a data-toggle="tooltip" title="' . $this->lang->line('Edit form') . '" href="' . base_url("messenger_bot_connectivity/edit_webview/{$info[$i]['canonical_id']}/1") . '" class="btn btn-circle btn-outline-warning"><i class="fa fa-edit"></i></a>';
    ...
    public function form_submit(){
       ...
       $select = array("facebook_rx_fb_page_info.facebook_rx_fb_user_info_id","facebook_rx_fb_page_info.page_id","page_access_token","first_name","last_name","messenger_bot_subscriber.status","facebook_rx_fb_page_info.id","messenger_bot_subscriber.user_id","messenger_bot_subscriber.id as subscriber_table_id");
       ...
       if(!empty($subscriber_info))
       {
	      ...
	      if($label_ids!=""){
	   		...
	   		$post_data_label_assign=array("psid"=>$subscriber_id,"fb_page_id"=>$page_id,"label_auto_ids"=>$label_ids,"subscriber_table_id"=>$subscriber_info[0]['subscriber_table_id']);
	      ...
	      if($text_reply_unique_id != '')
	      {
            ...
            // echo "<pre>"; print_r($data); exit;
	    		...
	    		if(!empty($form_data_mail)) {
	    			$str.= '<table cellpadding="0" cellspacing="0" width="640" align="center" border="1">
	    			<tr>
	    			<th scope="col" style="padding:5px; font-family: Arial,sans-serif; font-size: 16px; line-height:20px;line-height:30px">Field</th>
	    			<th scope="col" style="padding:5px; font-family: Arial,sans-serif; font-size: 16px; line-height:20px;line-height:30px">value</th>
	    			</tr>';


	    			foreach($form_data_mail as $head_key => $body_value) {
	    				if(is_array($body_value)) $body_value = implode(',',$body_value);
	    				$str.= '<tr>
	    				<td valign="top" style="padding:5px 10px; font-family: Arial,sans-serif; font-size: 14px; line-height:20px;">'.$head_key.'</td>
	    				<td valign="top" style="padding:5px 10px; font-family: Arial,sans-serif; font-size: 14px; line-height:20px;">'.$body_value.'</td>
	    				</tr>';
	    			}
	    			$str .= '</table>';
	    		}

	    		$message = str_replace(array('#APP_NAME#','#APP_URL#','#SUBSCRIBER_NAME#','#FORM_TITLE#','#FORM_DATA#'),array($mask,$url,$subscriber_name,$form_title,$str),$form_submit_email_template[0]['message']);
		...
		foreach ($form_data as $key => $form) {
			...
			switch ($form['type']) {
				...
				case 'radio-group':
					...
					$output .= '<div class="custom-switches-stacked mt-2' . ($required ? ' is_required_group' : null) . '">';
				...
				case 'checkbox-group':
					...
					$output .= '<div class="selectgroup selectgroup-pills' . ($required ? ' is_required_group' : null) . '">';
	...
	*/
	...
	public function find_page_postback()
	{
	  ...
	  $postback_infos = $this->basic->get_data("messenger_bot_postback",array('where'=>array('page_id'=>$pageID,'user_id'=>$user_id)),['template_name','postback_id']);
	  $webview_infos = $this->basic->get_data("webview_builder",array('where'=>array('page_id'=>$pageID,'user_id'=>$user_id)),['canonical_id','form_name']);
	  ...
	  $elements['html1'] = '<div class="form-group">
	              ...
	              { 
	                ...
	                {
	                    $elements['html1'] .="<option value='trigger_postback_{$postback['postback_id']}'>{$postback['template_name']}</option>";
	                }
	              } else
	              {
	                      ...
	                      <div class="form-group">
	                        ...
	                        { 
	                          ...
	                          { 
	                            ...
	                            $update_form .="<option value='trigger_postback_{$postback['postback_id']}' {$postbackchecked}>{$postback['template_name']}</option>";
	                          }
