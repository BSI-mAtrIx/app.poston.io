    ...
    public function business_accounts() 
    {
        ...
        if(!empty($login_config))
        {
            try
            {
                ...
                $data["google_login_button"] = $this->google_my_business_new->login_button();
            } catch (\Exception $e) {
...
'.$review_url.'
    ...
    public function reply_to_review()
    {
        ...
        $reviewer_location_name = $this->db->escape($reviewer_location_name);
        $review_id = $this->db->escape($review_id);
        $review_star = $this->db->escape($review_star);
        $review_comment = $this->db->escape($review_comment);
        $review_reply_message = $this->db->escape($review_reply_message);
        $reply_time = $this->db->escape($reply_time);

        if ('location_manager_index' == $reply_type) {
            $reviewer_display_name = $this->db->escape($reviewer_display_name);
            $reviewer_profile_photo = $this->db->escape($reviewer_profile_photo);

            $sql = <<<SQL
INSERT INTO `google_review_reply_report` (
    user_id
    ,location_id
...
UPDATE 
    review_reply = {$review_reply_message}
    ,location_display_name = {$reviewer_location_name}
    ,reviewer_name = {$reviewer_display_name}
    ,reviewer_photo = {$reviewer_profile_photo}
SQL;
        } else {
            $sql = <<<SQL
INSERT INTO google_review_reply_report (
...
SQL;
    ...
    public function question_list()
    {
        ...
        $location_table_id = $this->session->userdata('location_list_location_table_id');
        $location_info = $this->basic->get_data('google_business_locations',['where'=>['id'=>$location_table_id]]);
        $location_id = isset($location_info[0]['location_id']) ? $location_info[0]['location_id'] : '';

        $explode_location_id = explode('locations', $location_id);
        $location_id_segment = isset($explode_location_id[1]) ? $explode_location_id[1] : null;

        if (! $location_id_segment) {
            throw new Exception("Could not figure out location name for questions list.");
        }

        // Holds questions list
        $question_list = null;
        $question_list_error = null;

        try {
            $question_list = $gmb->questionsList("locations{$location_id_segment}/questions");
        } catch (\Exception $e) {
            $question_list_error = $e->getMessage();
        }
        ...
        if (isset($question_list['questions'])) {
            foreach ($question_list['questions'] as $key => $question) {
                ...
                if (is_object($question->getAuthor())) {
                    ...
                    $profilePhotoUrl = $author->getProfilePhotoUri();
                }
                    ...
                    foreach ($question->getTopAnswers() as $topAnswer) {
                        ...
                        if (is_object($topAnswer)) {
                            ...
                            $questionerAuthorInfo = [
                                ...
                                'profilePhotoUrl' => $questionerAuthor->getProfilePhotoUri(),
                            ];
                        }
    ...
    public function answer_to_question()
    {
        ...
        $this->form_validation->set_rules('question_id', 'required');
    ...
    public function posts_data()
    {
        ...
        for($i = 0; $i < count($info); $i++) {
            ...
            if ('event_post' == $post_type) {
                $info[$i]['title'] = $info[$i]['post_title'];
            }
        ...
        $html = '
            ...
            foreach ($posted_locations as $key => $value) {
                ...
                if(isset($value['error_message']) && !empty($value['error_message'])) {
                    ...
                    $error_message_array = is_string($value['error_message']) ? json_decode($value['error_message'], true) : null;

                    $error_message1 = '';
                    $error_message2 = '';

                    if (is_array($error_message_array)) {
                        $error_message1 = isset($error_message_array['error']['message'])
                            ? $error_message_array['error']['message']
                            : '';
                        $error_message2 = isset($error_message_array['error']['details'][0]['errorDetails'][0]['message'])
                            ? $error_message_array['error']['details'][0]['errorDetails'][0]['message']
                            : '';
                    }

                    $error_message = $error_message2 ? $error_message2 : $error_message1;

                    $html .= '<a data-toggle="tooltip" title="'.$error_message.'" href="#" class="btn btn-sm btn-danger mt-2 mb-2 mr-2 ml-0"><i class="fas fa-exclamation-circle"></i> '.$value['location_name'].'</a>';
    ...
    public function create_campaign()
    {
        ...
        if ('cta_post' == $this->input->post('submitted_post_type')) {
            ...
            $this->form_validation->set_rules('post_title', $this->lang->line('Post title'), 'required');
