<?php
/*
Addon Name: NVX Image Editor MiniPaint
Unique Name: nvx_minipaint
Modules:
{
   "3005":{
...
Version: 1.22
Description: Image Editor MiniPaint
*/


require_once("application/controllers/Home.php"); // loading home controller
class N2_image_editor extends Home
{
	public $key = "B08CEFF60216DE3E";
	private $product_id = "3";
	private $product_base = "nvx_minipaint";
	private $server_host = "https://nvxgroup.com/wp-json/licensor/";
	private $nvx_version = 1.22;
	/* @var self*/
        ...
        if($function_name!="webhook_callback")
        {
             // all addon must be login protected
              ...
              //-------------------------------------------------------------------------------------------
                ...
                if($this->session->userdata('user_type') != 'Admin' && !in_array(3005,$this->module_access))
                   {
    ...
    public function delete(){        
        ...
        $sql=array
        (
          0 => "DELETE from `menu` where url like '%n2_image_editor%' ",
        );  
	...
	public function activate()
    {
        ...
        if(is_dir(APPPATH.'modules/n2_image_editor/miniPaint/')){
         ...
         $menu_exists = $this->db->query(" SELECT id FROM `menu` where url LIKE '%n2_image_editor%' ")->row_array();
        $parent_id_to_add = $this->db->query(" SELECT serial FROM `menu` where url LIKE '%ultrapost%' ")->row_array();
        if(!$menu_exists){
            try{
                $sql_cust = "INSERT INTO `menu` (`id`, `name`, `icon`, `url`, `serial`, `module_access`, `have_child`, `only_admin`, `only_member`, `add_ons_id`, `is_external`, `header_text`, `is_menu_manager`, `custom_page_id`) VALUES (NULL, 'Image Editor N2', 'fa fa-image', 'n2_image_editor/', ".$parent_id_to_add['serial'].", '3005', '0', '0', '0', '0', '0', '', '0', '0')" ;
...
//         //send blank array if you does not need sidebar entry,send a blank array if your addon does not need any sql to run
    ...
    public function fix_menu(){
        $sql_cust = "DELETE from `menu_child_1` where url like '%n2_image_editor%' " ;
        $this->db->query($sql_cust);
    ...
    public function index(){
		if($this->session->userdata('user_type') != 'Admin' && !in_array(3005,$this->module_access)) redirect('home/login', 'location');
		$data['body'] = 'image_editor';
        $data['page_title'] = $this->lang->line('Image editor');
