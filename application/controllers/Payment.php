...
class Payment extends Home
{
    ...
    { 
        ...
        if($n_config['dp_on'] == 'true'){
            $display_columns = array("#","CHECKBOX",'id','receiver_email','first_name', 'last_name', 'payment_type', 'cycle_start_date','cycle_expired_date', 'payment_date','paid_amount', 'n_invoice');
        }
            ...
            if($this->session->userdata('user_type')=="Admin") {
                ...
                if($n_config['dp_on'] == 'true'){
                    if(!empty($value['paid_amount'])){
                        if(empty($pck_currency[$value['package_id']])){
                            $pck = $this->basic->get_data('package', ['where' => ['id' => $value['package_id']]], '', '', 1);
                            $pck_currency[$value['package_id']] = $pck[0]['currency'];
                        }
                        $info[$i]["paid_amount"] = $value['paid_amount'].' '.$pck_currency[$value['package_id']];
                    }

                    if(!empty($info[$i]["n_invoice"])){
                        $info[$i]["n_invoice"] = '<i class="bx bx-check-shield"></i>';
                    }

                        $info[$i]["n_invoice"] .= '<a href="#" class="open_invoice_admin" data-id_trans="'.$value['id'].'" data-id_pack="'.$value['package_id'].'"><i class="bx bxs-file-pdf"></i></a>';
                ...
                if($n_config['dp_on'] == 'true'){
                    ...
                    if(!empty($info[$i]["n_invoice"])){
                        $info[$i]["n_invoice"] = '<a href="'.base_url().$value['n_invoice'].'" target="_blank"><i class="bx bxs-file-pdf"></i></a>';
                    }
                }
    ...
    public function buy_package($dynamic = 0)
    {
        if($this->session->userdata('license_type') == 'double' && $this->session->userdata('user_type') == 'Member')
        {
           ...
           $data['payment_method'] = isset($payment_method[0]['payment_type']) ? $payment_method[0]['payment_type'] : 'Paypal';
            ...
            $where=array("where"=>array("is_default"=>"0","price > "=>0,"validity >"=>0,"visible"=>"1"));
           $data["payment_package"]=$this->basic->get_data("package",$where,$select='',$join='',$limit='',$start=NULL,$order_by='CAST(`price` AS SIGNED)');
    ...
    public function transaction_log_manual_data() 
    {
        ...
        if('Admin' == $this->session->userdata('user_type')) {
            $where_custom = "`transaction_history_manual`.user_id > 0 ";
        } else {
            $where_custom = "`transaction_history_manual`.user_id = " . $this->user_id;
        }
    ...
    {      
        ...
        if ($this->session->userdata('license_type') == 'double' && $this->session->userdata('user_type') == 'Member') 
        {
            ...
            if($n_config['dp_on'] == 'true'){
                $currency=$package_data[0]['currency'];
            }
        ...
        if($myfatoorah_api_key != '')
        {
            ...
            if($n_config['dp_on'] == 'true'){
                $button_size_new = 'col-12';
                $package_data[0]['return_json'] = json_decode($package_data[0]['return_json'], true);
                if(!empty($package_data[0]['return_json']['ps']['pm'])){
                    $package_data[0]['return_json']['ps']['pm'] = json_decode($package_data[0]['return_json']['ps']['pm'],true);
                    $pms = array();
                    foreach($package_data[0]['return_json']['ps']['pm'] as $k => $v){
                        $pms[$v] = 1;
                        switch($v){
                            case 'pp_button';
                                if($pp_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$pp_button.'</div>';
                                break;
                            case 'st_button';
                                if($st_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$st_button.'</div>';
                                break;
                            case 'razorpay_button';
                                if($razorpay_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$razorpay_button.'</div>';
                                break;
                            case 'paystack_button';
                                if($paystack_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$paystack_button.'</div>';
                                break;
                            case 'mollie_button';
                                if($mollie_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$mollie_button.'</div>';
                                break;
                            case 'mercadopago';
                                if($mercadopago != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$mercadopago.'</div>';
                                break;
                            case 'sslcommerz';
                                if($sslcommerz != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$sslcommerz.'</div>';
                                break;
                            case 'senangpay';
                                if($senangpay != '') $buttons_html .= '<div class="'.$button_size_new.'pt-3">'.$senangpay.'</div>';
                                break;
                            case 'instamojo';
                                if($instamojo != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$instamojo.'</div>';
                                break;
                            case 'toyyibpay';
                                if($toyyibpay != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$toyyibpay.'</div>';
                                break;
                            case 'myfatoorah';
                                if($myfatoorah != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$myfatoorah.'</div>';
                                break;
                            case 'paymaya_button';
                                if($paymaya_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$paymaya_button.'</div>';
                                break;
                            case 'xendit';
                                if($xendit != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$xendit.'</div>';
                                break;
                        }
                    }
                    $data['pms'] = $pms;
                ...
                $button_size_new = 'col-12 col-md-4 pt-3';
                if($pp_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$pp_button.'</div>';
                if($st_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$st_button.'</div>';
                if($razorpay_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$razorpay_button.'</div>';
                if($paystack_button != '') $buttons_html .= '<div class="'.$button_size_new.'">'.$paystack_button.'</div>';
            ...
            if($n_config['dp_on'] == 'true'){
                $data["payment_package"]=$this->basic->get_data("package",
                    $where=array("where"=>
                        array("is_default"=>"0","validity >"=>0,"user_id"=>$this->user_id,"id"=>$package_id)
                    )
                    ,$select='',$join='',$limit='',$start=NULL,$order_by='CAST(`price` AS SIGNED)');
                if(empty($data["payment_package"])){
                    $data["payment_package"]=$this->basic->get_data("package",
                        $where=array("where"=>
                            array("is_default"=>"0","validity >"=>0,"visible"=>"1","id"=>$package_id)
                        )
                        ,$select='',$join='',$limit='',$start=NULL,$order_by='CAST(`price` AS SIGNED)');
                }
            }else{
                $data["payment_package"]=$this->basic->get_data("package",
                    $where=array("where"=>
                        array("is_default"=>"0","price > "=>0,"validity >"=>0,"visible"=>"1","id"=>$package_id)
                    )
                    ,$select='',$join='',$limit='',$start=NULL,$order_by='CAST(`price` AS SIGNED)');
            }
            if(!empty($data['payment_package'][0]['return_json']) AND $n_config['dp_on'] == 'true'){
                $data['body'] = "member/buy_buttons_dynamic";

                $ock = $this->basic->get_data('price_dynamic_plans', ['where' => ['id' => $package_data[0]["origin_id"]]], '', '', 1);
                if(!empty($ock[0])){
                    $ock[0]['config'] = json_decode($ock[0]['config'],true);
                    if($ock[0]['config']['vat']['vat_collect']=='1'){
                        $data['body'] = "member/buy_buttons_dynamic";
                    }else{
                        $data['body'] = "member/buy_buttons_dynamic_without";
                    }
                }
            }else{
            ...
            $user_info = $this->basic->get_data('users', array('where' => array('id' => $this->user_id)), array('paypal_subscription_enabled', 'last_payment_method', 'invoice_data'));
            if(!isset($user_info[0])) exit();

            $data['user_invoice'] = json_decode($user_info[0]['invoice_data'], true);
            if($user_info[0]['paypal_subscription_enabled'] == '1' ) $data['has_reccuring'] = 'true';
            else $data['has_reccuring'] = 'false';
            $data['last_payment_method'] = $user_info[0]['last_payment_method'];
            $this->_viewcontroller($data);
