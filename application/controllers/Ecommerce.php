...
{    
  ...
  public function __construct()
  {
	...
	$this->load->library('tap_ecommerce');
  ...
  public function tap_payment_ecommerce($tap_secret_api_key, $amount, $user_id, $first_name, $last_name, $email, $store_id, $cart_id, $subscriber_id, $payment_amount, $currency, $store_name){
        
        // echo $this->tap->action_url;
        $this->tap_ecommerce->payment($tap_secret_api_key, $amount, $user_id, $first_name, $last_name, $email, $store_id, $cart_id, $subscriber_id, $payment_amount, $currency, $store_name);
    }
	
  public function tap_callback($tap_secret_api_key, $store_id, $cart_id, $subscriber_id, $payment_amount, $currency, $store_name){
        
        $res = $this->tap_ecommerce->callback($_GET['tap_id'], $tap_secret_api_key);
        //echo "<pre>";
        //print_r($res);
        //echo "</pre>";
        $this->tap_action($res, $store_id, $cart_id, $subscriber_id, $payment_amount, $store_name);
    }
    
	
	public function upload_products(){       
		
		  $data['body']='ecommerce/upload_ecom_products';     
		  $data['page_title']=$this->lang->line('Upload Products')." : ".$this->session->userdata("ecommerce_selected_store_title");
	...
	public function tap_action($res, $store_id, $cart_id, $subscriber_id, $payment_amount, $store_name)
	{
		$response= $res;
		...
		$receiver_email = $response->customer->email;
	//  if($currency=='JPY' || $currency=='VND') $payment_amount=$response->amount;
	//  else $payment_amount=$response->amount;
		$payment_amount = $response->amount;
		$transaction_id = $response->id;
		$payment_date= date("Y-m-d H:i:s",$response->transaction->created);
		$tap_card_source=isset($response->source)?$response->source:"";
		$tap_card_source=json_encode($tap_card_source);
		$country = "";
		...
		"checkout_source_json"=>$tap_card_source,            
		'paid_at' => $curtime,
		'status' => 'approved',
...
$Tap_enabled = $cart_data[0]["Tap_enabled"];
...
$tap_secret_api_key = isset($ecommerce_config['tap_secret_api_key']) ? $ecommerce_config['tap_secret_api_key'] : '';
...
if($Tap_enabled=='1'  && $tap_secret_api_key=='') 
{
  echo json_encode(array('status'=>'0','message'=>'Tap payment settings not found.'));
  exit();
}
...
if($Tap_enabled=="1")
{
    $this->tap_ecommerce->currency=$currency;
    $this->tap_ecommerce->first_name=$customer_first_name;
    $this->tap_ecommerce->last_name=$customer_last_name;
    if($customer_email!="")$this->tap_ecommerce->email=$customer_email;
    else $this->tap_ecommerce->email=$fake_email;
    
    $this->tap_ecommerce->amount=$payment_amount;
    $tap_button = $this->tap_ecommerce->set_button($user_id,$store_id, $cart_id, $subscriber_id, $payment_amount, $currency, $store_name);
	$tap_button = '<div class="col-12 col-md-6">'.$tap_button.'</div>';
}
...
$html ='<div class="row">'.$cod_button.$paypal_button.$tap_button.$stripe_button.$razorpay_button.$paystack_button.$mollie_button.$mercadopago_button.$sslcommerz_button.$senangpay_button.$instamojo_button.$xendit_button.$manual_button.$toyyibpay_button.$myfatoorah_button.$paymaya_button.'</div>';
...
public function payment_accounts_action()
{
  ...
  if ($_POST) 
  {
	...
	$this->form_validation->set_rules('tap_secret_api_key','<b>'.$this->lang->line("toyyibpay_secret_key").'</b>','trim');
    $this->form_validation->set_rules('Tap_enabled','<b>'.$this->lang->line("paymaya_enabled").'</b>','trim');
    ...
    else 
    {
	  ...
	  $tap_secret_api_key=strip_tags($this->input->post('tap_secret_api_key',true));
      $Tap_enabled=strip_tags($this->input->post('Tap_enabled',true));
      ...
      if($paypal_enabled=='0' && $stripe_enabled=='0' && $manual_enabled=='0' && $cod_enabled=='0' && $razorpay_enabled=='0' && $paystack_enabled=='0' && $mollie_enabled=='0' && $mercadopago_enabled=='0' && $sslcommerz_enabled=='0' && $senangpay_enabled=='0' && $instamojo_enabled=='0' && $xendit_enabled=='0' && $toyyibpay_enabled == '0' && $myfatoorah_enabled == '0' && $paymaya_enabled == '0' && $Tap_enabled == '0')
      {
        $this->session->set_flashdata('error_message', 1);
      ...
      array
      (
		'tap_secret_api_key'=>$tap_secret_api_key,
        ...
        // $this->form_validation->set_rules('store_id', '<b>'.$this->lang->line("Store").'</b>', 'trim|required');      
            ...
            {        
              ...
              { 
		       ...
		       $update_data=array("time_zone"=>$time_zone);
				$res= $this->basic->update_data("users",array("id"=>$this->session->userdata("user_id")),$update_data);
