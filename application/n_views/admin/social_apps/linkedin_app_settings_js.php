<script>
    ...
    $(document).ready(function () {
        ...
        var table = $("#mytable").DataTable({
            ...
            pageLength: 10,
            ajax: {
                url: base_url + 'social_apps/linkedin_settings_data',
                type: 'POST'
            },
            language:
                {
        ...
        $(document).on('click', '.delete_app', function (e) {
            e.preventDefault();
            var ifyoudeletethisaccount = "<?php echo $this->lang->line('If you delete this APP then, all the imported Linkedin accounts and Campaigns will be deleted too corresponding to this APP.'); ?>";
            swal.fire({
                ...
                .then((willDelete) => {
                    if (willDelete.isConfirmed) {
                        ...
                        $.ajax({
                            context: this,
                            type: 'POST',
                            url: "<?php echo site_url();?>social_apps/delete_app_linkedin",
                            dataType: 'json',
        ...
        $(document).on('click', '.change_state', function (e) {
            e.preventDefault();
            var ifyoudeletethisaccount = "<?php echo $this->lang->line('If you change this APP status to inactive then, all the imported Linkedin accounts and Campaigns will not work corresponding to this APP.'); ?>";
            swal.fire({
                title: '<?php echo $this->lang->line("Are you sure?"); ?>',
                text: ifyoudeletethisaccount,
                icon: 'warning',
                ...
                .then((willDelete) => {
                    if (willDelete.isConfirmed) {
                        ...
                        $.ajax({
                            context: this,
                            type: 'POST',
                            url: "<?php echo site_url();?>social_apps/change_app_status_linkedin",
                            dataType: 'json',
