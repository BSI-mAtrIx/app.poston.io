...
if ($n_eco_builder_config['new_products_show'] == 'true') {
    if (!empty($new_list)) {
        ...
        <div class="title-link-wrapper pb-1 mb-4">
            <h2 class="title ls-normal mb-0"><?php echo $n_eco_builder_config['new_products_text']; ?></h2>
            <?php
            $n_special_generate = _link('ecommerce/new_products/' . $js_store_unique_id);
    ...
    <div class="<?php columns_width($n_eco_builder_config['new_products_rows'], $n_eco_builder_config['mobile_columns_div']); ?>">

        <?php
        $hide_reviews_listing = 'true';
        $always_show_reviews = 'false';
        if ($n_eco_builder_config['new_products_reviews_show'] == 'show' or $n_eco_builder_config['new_products_reviews_show'] == 'always_show') {
            $hide_reviews_listing = 'false';
        }
        if ($n_eco_builder_config['new_products_reviews_show'] == 'always_show') {
