...
<script type="text/javascript">
    ...
    function load_data(start, reset, popmessage, comment_id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.html = response.html.replaceAll('<h6 class="mt-1 mb-0">', '<h4 class="media-heading mb-0 pb-0 float-left">');
                response.html = response.html.replaceAll('<p class="m-0 small d-inline">', '<p class="float-right font-small-3 d-inline">');
                response.html = response.html.replaceAll('<a target', '<a class="text-bold-500" target');
                response.html = response.html.replaceAll('<a class="collpase_link', '<a class="collpase_link clearn');
                response.html = response.html.replaceAll('<p class="mb-0">', '<p class="mb-0 clearn" style="width:75%;display:inline-block;">');
                response.html = response.html.replaceAll('fas fa-reply', 'bx bx-reply');
                response.html = response.html.replaceAll('fas fa-eye-slash', 'bx bxs-hide');
                response.html = response.html.replaceAll('fas fa-comment', 'bx bx-comment');
                ...
                if (response.found != '0') $("#load_more").show();
                    ...
                    if (popmessage) {
                        iziToast.info({
                            title: '',
                            message: "<?php echo $this->lang->line('No more comment found.') ?>",
    ...
    $(document).ready(function () {
        $(document).on('click', '.leave_comment', function (e) {
            ...
            if (new_comment == "") {
                swal.fire("<?php echo $this->lang->line('Error'); ?>", "<?php echo $this->lang->line('Please write a comment.'); ?>", 'error');
                return false;
            }
                    ...
                    if (willDelete.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                if (response.status == '0') swal.fire({
                                    title: '<?php echo $this->lang->line("Error"); ?>',
                                    html: span,
                                    icon: 'error'
                                });
        ...
        $(document).on('click', '#rate_now', function (e) {
            ...
            if (reason == '' || rating == '' || cart_id == '') {
                swal.fire('<?php echo $this->lang->line("Error"); ?>', '<?php echo $this->lang->line("Please fill in the required fields"); ?> *', 'error');
                return false;
            }
            $(this).addClass('btn-progress');
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == '0') {
                        ...
                        swal.fire('<?php echo $this->lang->line("Review Submitted"); ?>', response.message, 'success').then((value) => {
                            location.reload();
                        });
                    }
                }
                    ...
                    if (willDelete.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                else swal.fire('<?php echo $this->lang->line("Hidden Successfully"); ?>', response.message, 'success').then((value) => {
                                    location.reload();
                                });
                            }
                        });
        ...
        $(document).on('click', '.leave_review_comment', function (e) {
            ...
            if (review_reply == "") {
                swal.fire("<?php echo $this->lang->line('Error'); ?>", "<?php echo $this->lang->line('Please write a reply.'); ?>", 'error');
                return false;
            }
            $(this).addClass('btn-progress');
            $.ajax({
                ...
                data: {
                    ...
                    subscriber_id: subscriber_id,
                    parent_product_review_id: parent_product_review_id
                },
                success: function (response) {
                    $(this).removeClass('btn-progress');
                    if (response.status == '0') swal.fire('<?php echo $this->lang->line("Error"); ?>', response.message, 'error');
