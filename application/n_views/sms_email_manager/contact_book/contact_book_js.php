...
<script>
    ...
    $(document).ready(function ($) {
        ...
        $(document).on('click', '#save_group', function (event) {
            ...
            if (group_name == '') {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Group Name is Required"); ?>', 'warning');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response == "1") {
                        iziToast.success({
                            title: '',
                            message: '<?php echo $this->lang->line('Contact Group has been created successfully.'); ?>',
                            position: 'bottomRight'
                        });
                    } else if (response == "2") {
                        iziToast.error({
                            title: '',
                            message: '<?php echo $this->lang->line('Group Name Already Exists, please try with different one.'); ?>',
                            position: 'bottomRight'
                        });
                    } else {
                        iziToast.error({
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response == "1") {
                        iziToast.success({
                            title: '',
                            message: '<?php echo $this->lang->line('Contact Group has been Updated successfully.'); ?>',
                            position: 'bottomRight'
                        });
                    } else if (response == "2") {
                        iziToast.error({
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response == '1') {
                                    iziToast.success({
                                        title: '',
                                        message: '<?php echo $this->lang->line('Contact Group has been deleted successfully.'); ?>',
                                        position: 'bottomRight',
                                        timeout: 3000
                                    });
        ...
        $(document).on('click', '#save_contact', function (event) {
            ...
            if (contact_email == "" && phone_number == "") {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Email/Phone number is Required"); ?>', 'warning');
                return;
            }
            ...
            if (contact_group_name == "" || contact_group_name == null || typeof (contact_group_name) == "undefined") {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Contact Group is Required"); ?>', 'warning');
                return;
            }
        ...
        $(document).on('click', '#export_contact', function (event) {
            ...
            if (contact_ids.length == 0) {
                swal.fire('<?php echo $this->lang->line("Warning")?>', '<?php echo $this->lang->line("You have to select Contacts to export.") ?>', 'warning');
                return false;
            } else {
                $(this).addClass('btn-progress');
                $.ajax({
        ...
        $(document).on('click', '#assign_sms_email_sequence', function (event) {
            ...
            if (ids == "") {
                swal.fire('<?php echo $this->lang->line("Warning") ?>', '<?php echo $this->lang->line("You have to select Contacts to assign sequence.") ?>', 'warning');
                return;
            }
        ...
        $(document).on('click', '#assign_sequence_submit', function (e) {
            ...
            if (count == 0) {
                swal.fire('<?php echo $this->lang->line("Error") ?>', '<?php echo $this->lang->line("You have to select campaign to assign.") ?>', 'error');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    swal.fire('<?php echo $this->lang->line("Sequence Campaign Assign") ?>', '<?php echo $this->lang->line("Sequence campaign have been assigned successfully"); ?>', 'success')
                        .then((value) => {
        ...
        $(document).on('click', '#delete_all_contacts', function (event) {
            ...
            if (contact_ids.length == 0) {
                ...
                swal.fire('<?php echo $this->lang->line("Warning")?>', '<?php echo $this->lang->line("You didn`t select any Contact to delete.") ?>', 'warning');
                return false;
        ...
        $(document).on('click', '#assign_manual_sequence_submit', function (event) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    iziToast.success({
                        title: '',
                        message: '<?php echo $this->lang->line("Contact information has been saved successfully."); ?>',
                        position: 'bottomRight'
                    });
                }
            })
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response == "unsubscribed") {
                        ...
                        iziToast.success({
                            title: '',
                            message: '<?php echo $this->lang->line("Contact has been successfully unsubscribed.") ?>',
                            position: 'bottomRight'
                        });
                        ...
                        iziToast.success({
                            title: '',
                            message: '<?php echo $this->lang->line("Contact has been successfully subscribed.") ?>',
                            position: 'bottomRight'
                        });
                        ...
                        iziToast.error({
                            title: '',
                            message: '<?php echo $this->lang->line("Something went wrong, try once again.") ?>',
                            position: 'bottomRight'
                        });
                    }
                }
        ...
        $(document).on('click', '#upload_imported_csv', function (event) {
            ...
            if (csvFile == "") {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Please Upload Contact CSV file."); ?>', 'warning');
                return;
            }
