...
<script>
    $(document).ready(function ($) {
        ...
        $(document).on('click', '#save_api', function (event) {
            ...
            if (gateway_name == "") {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Gateway is Required"); ?>', 'warning');
                return;
            }

            /* Check that Routesms.com hostname is valid or not */
            if (gateway_name == "routesms.com" && !isURL(routesms_host_name)) {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Route host URL is not valid."); ?>', 'warning');
                return;
            }
            ...
            if (namesForAuthId.indexOf(gateway_name) != -1 && (username_auth_id == "" || phone_number == "")) {
                ...
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("You didn`t Provided all required information of the Gateway"); ?>', 'warning');
                return;
            }

            if (gateway_name == "clickatell" && username_auth_id == "") {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("API Username is Required"); ?>', 'warning');
                return;
            }

            /* checking host name for routesms.com gateway */
            if (gateway_name == "routesms.com" && routesms_host_name == "") {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Routesms Host name is required."); ?>', 'warning');
                return;
            }
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                if (response == '1') {
                                    iziToast.success({
                                        title: '',
                                        message: '<?php echo $this->lang->line('API has been Deleted Successfully.'); ?>',
        ...
        $("#import_submit").click(function () {
            ...
            if (fileval == "")
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("No file selected, Please upload a file.");?>', 'warning');
            else {
                $(this).addClass('btn-progress')
                var that = $(this);
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status == '1') {
                            ...
                            $("#manual_numbers").html('<i class="bx bx-loader-alt bx-spin"></i>');
                            $("#manual_numbers").html(totalNumbers);
                            $("#to_numbers").val(file_content);
                            $("#csv_import_modal").modal('hide');
                            iziToast.success({
                                title: '',
                                message: '<?php echo $this->lang->line("import from csv was successful")?>',
                                position: 'bottomRight'
                            });
                        } else {
        ...
        $(document).on('click', '#create_campaign', function () {
            ...
            // campaign name
            if (campaign_name == '') {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', "<?php echo $this->lang->line("Please give a campaign name"); ?>", 'warning');
                return;
            }

            // sms api select
            if (from_sms == '') {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', "<?php echo $this->lang->line("Please select a SMS API"); ?>", 'warning');
                return;
            }

            // write message
            if (message == '') {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', "<?php echo $this->lang->line("Please write your message"); ?>", 'warning');
                return;
            }

            if (page_name == "" && contacts_id == "" && to_numbers == "") {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', "<?php echo $this->lang->line("Please Select Page name or Contact Group."); ?>", 'warning');
                return;
            }
        ...
        $(document).on('click', '#updateMessage', function (event) {
            ...
            var message = $("#message").val();
            if (message == "") {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', "<?php echo $this->lang->line("Please type a message. System can not send blank message."); ?>", 'warning');
                return;
                ...
                success: function (response) {
                    ...
                    if (response == "1") {
                        swal.fire({
                            ...
                            content: '<?php echo $this->lang->line("Campaign have been updated successfully."); ?>',
                            icon: 'success'
                        }).then((value) => {
                            window.location.href = report_link;
                        });
        ...
        $(document).on('change', '#contacts_id', function (event) {
            ...
            $("#contact_numbers").html('<i class="bx bx-loader-alt bx-spin"></i>');
        ...
        $(document).on('click', '#analyze_button', function (event) {
            ...
            $("#analyzed_section").html('<div class="text-center waiting"><i class="bx bx-loader-alt bx-spin blue text-center" style="font-size:40px"></i></div>');
            ...
            if (custom_url != '') {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        table_id: table_id,
                        is_first_time_edit_request: is_first_time_edit_request
                    },
                    success: function (response) {
                        ...
                        if (response.message == 'error') {
                            ...
                            swal.fire('<?php echo $this->lang->line("Error"); ?>', '<?php echo $this->lang->line("Please provide a valid url"); ?>', 'error');
                            $("#analyzed_section").html('');
                        } else {
                ...
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Please put the input first."); ?>', 'warning');
                $("#analyzed_section").html('');
            }
        ...
        $(document).on('click', '#update_custom_api', function (event) {
            ...
            if (custom_api_name == '' || custom_api_url == '' || updated_url == '') {
                swal.fire('<?php echo $this->lang->line("Error"); ?>', '<?php echo $this->lang->line("Either api name or api url is empty."); ?>', 'error');
            } else {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status == 'success') {
                            ...
                            swal.fire('<?php echo $this->lang->line("Success"); ?>', response.message, 'error');
                        }
        ...
        $(document).on('click', '#update_custom_api_post_method', function (event) {
            ...
            if (custom_api_name == '' || custom_api_url == '') {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        custom_api_name_post_method: custom_api_name,
                        custom_api_base_url_post_method: custom_api_url,
                        action_type: action_type,
                        table_id: table_id,
                        key: key,
                        types: types,
        ...
        $(document).on('click', '#text_response', function (event) {
            ...
            $("#text_response_section").html('<div class="text-center waiting"><i class="bx bx-loader-alt bx-spin blue text-center" style="font-size:40px"></i></div>');
            ...
            if (custom_api_base_url_post_method != '') {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        custom_api_name_post_method: custom_api_name_post_method,
                        custom_api_base_url_post_method: custom_api_base_url_post_method,
                        action_type: action_type,
                        table_id: table_id,
                        is_first_time_edit_request: is_first_time_edit_request,
                        key: key,
                ...
                success: function (response) {
                    ...
                    if (response.status == 'success') {
                        ...
                        response.postData.forEach((item, index) => {
                            ...
                            let show = (index === 0) ? '' : '<button class="btn btn-danger close_data"><i class="bx bx-x-circle"></i></button>';
                            ...
                            if (parameter_type == "DESTINATION_NUMBER" || parameter_type == "destination_number") destination_selected = "selected";
