        ...
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h3 class="modal-title text-center"><i
                            class="bx bx-check-circle"></i> <?php echo $this->lang->line("Enable Auto Like/Share"); ?>
                </h3>
            ...
            <div class="modal-footer text-center">
                <button class="btn btn-primary" id="autolikeshare_save_button"><i
...
<div class="modal fade" id="pageresponse_auto_likeshare_edit_modal" data-backdrop="static" data-keyboard="false">>
    <div class="modal-dialog" style="min-width: 60%;">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h3 class="modal-title text-center"><i
                            class="bx bx-edit"></i> <?php echo $this->lang->line("Edit Auto Like/Share"); ?></h3>
                <button type="button" class="close" data-dismiss="modal"><i class="bx bx-x"></i></button>
            </div>
            ...
            <!-- Modal footer -->
            <div class="modal-footer text-center">
                <button class="btn btn-primary" id="autolikeshare_edit_button"><i
                            class="bx bx-save"></i> <?php echo $this->lang->line('Save'); ?></button>
            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.enable_auto_share', function (e) {
            ...
            var loading = '<div class="text-center modal_waiting"><i class="bx bx-loader-alt bx-spin blue text-center"></i></div>';
            ...
            $.ajax({
                ...
                data: {
                    page_response_user_info_id: page_response_user_info_id,
        ...
        $(document).on('click', '#autolikeshare_save_button', function () {
            ...
            pageresponse_auto_likeshare_modal_body
            ...
            if (typeof (auto_share_post) == 'undefined' && typeof (auto_like_post) == 'undefined') {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Please enable auto share or auto like."); ?>', 'warning');
                return;
            }


            if (auto_share_post == '1' && auto_share_this_post_by_pages.length == 0) {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Please select page for auto sharing."); ?>', 'warning');
                return;
            }

            if (auto_like_post == '1' && auto_like_this_post_by_pages.length == 0) {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Please select page for auto liking."); ?>', 'warning');
                return;
            }
        ...
        $(document).on('click', '#autolikeshare_edit_button', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    $(this).removeClass('btn-progress');
                    if (response == "success") {
                        swal.fire('<?php echo $this->lang->line("Success"); ?>', '<?php echo $this->lang->line("campaign information has been saved successfully.");?>', 'success').then((value) => {
                            $(".page_list_item.active").click();
