<script>
    ...
    $(document).on('click', '.fullpage_pause_play', function (e) {
        ...
        if(to_do==undefined){
            to_do = $('#n_fullpage_pause_play .fullpage_pause_play').attr('pause_play');
        }
        var table_id = $(this).attr('table_id');
        if(table_id==undefined){
            table_id = $('#n_fullpage_pause_play .fullpage_pause_play').attr('table_id');
        }
    ...
    $(document).on('click', '.pageresponse_edit_reply_info', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#pageresponse_edit_generic_message_private").html(response.postbacks).select2({width: "100%"});
                $("#pageresponse_edit_ai_message_private").html(response.postbacks).select2({width: "100%"});
        ...
        if(response.ai_reply_enabled == '1')
        {
          ...
          $("#pageresponse_edit_ai_message_private").val(response.auto_reply_text).change();
        }
        else
        {
            ...
            $("#pageresponse_edit_ai_message_div").hide();
                    ...
                    $("#pageresponse_edit_generic_message").val(auto_reply_text_array[0]['comment_reply']).trigger('change');
                    $("#pageresponse_edit_generic_message_private").val(auto_reply_text_array[0]['private_reply']).trigger('change');
