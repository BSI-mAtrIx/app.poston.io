...
<script>
    ...
    $(document).ready(function () {
        ...
        window.location.href = base_url + 'messenger_bot/domain_whitelist/' + $('#bot_list_select').val();
        <?php } ?>
        ...
        $(document).on('click', '#add_new_domain_submit', function () {
            ...
            if (page_id == '' || domain_name == '') {
                swal.fire('<?php echo $this->lang->line("Warning"); ?>', "<?php echo $this->lang->line('Please select page and provide domain name.'); ?>", 'warning');
                return false;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == '1')
                        swal.fire('<?php echo $this->lang->line("Success"); ?>', response.result, 'success').then((value) => {
                            $("#add_new_domain_modal").modal('hide');
                            table.draw();
                        });
                    else
                        swal.fire('<?php echo $this->lang->line("Error"); ?>', response.result, 'error');
                }
            });
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                if (response == '1') {
                                    ...
                                    var deleted_html = "<span class='badge badge-light red'><i class='bx bx-check'></i> " + deleted + "</span>";
                                    iziToast.success({
                                        title: '',
                                        message: '<?php echo $this->lang->line("Domain has been deleted successfully."); ?>',
                                        position: 'bottomRight'
                                    });
                                    $("#" + col_id).parent().html(deleted_html);
                                    table1.draw();
                                } else {
                                    swal.fire('<?php echo $this->lang->line("Error"); ?>', "<?php echo $this->lang->line('Something went wrong, please try again later.'); ?>", 'error');
                                }
                            }
                        });
                    }
